...
// ── Generate report ───────────────────────────────────────────────────
export const generateReport = async (meetingId, generatedBy) => {
    ...
    const deptIds = [...new Set(staffList.map((s) => s.department_id).filter(Boolean))];
    let staffDeptMap = {};
    if (deptIds.length > 0) {
        ...
        if (depts) {
            staffDeptMap = Object.fromEntries(depts.map((d) => [d.department_id, d.name]));
        }
    }
...
// ── List reports ──────────────────────────────────────────────────────
export const listReports = async (userId, role, page = 1, limit = 20, status) => {
...
// ── Submit report to HR ───────────────────────────────────────────────
export const submitToHR = async (reportId, submittedBy) => {
    const { data: report, error } = await supabaseAdmin
        .from('reports')
    ...
    if (report.status === 'submitted_to_hr')
        throw new Error('Report already submitted to HR');
    // Update status
    ...
    const { data: meeting } = await supabaseAdmin
        ...
        .single();
    const organizerName = meeting?.profiles?.full_name ?? 'N/A';
    ...
    if (hrUsers && hrUsers.length > 0) {
        ...
        const hrNotifications = hrUsers.map((u) => ({
            user_id: u.id,
            related_meeting_id: report.meeting_id,
            related_report_id: reportId,
        ...
        for (const hrUser of hrUsers) {
            const tpl = templates.reportSubmittedToHR(hrUser.full_name, meetingTitle, organizerName, report.total_attendance, reportId);
