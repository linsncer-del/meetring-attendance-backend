...
export const createUser = async (input) => {
    ...
    sendMail({ to: input.email, subject: mailTpl.subject, html: mailTpl.html }).catch(err => console.error('[Mailer] Welcome email failed:', err));
    return profile;
};
// ── Update user ───────────────────────────────────────────────────────
export const updateUser = async (id, input) => {
    ...
    if (input.department_id !== undefined)
        updates.department_id = input.department_id;
    const { data, error } = await supabaseAdmin
        .from('profiles')
        .update(updates)
        .eq('id', id)
...
export const adminResetPassword = async (id, tempPassword) => {
    ...
    sendMail({ to: profile.email, subject: mailTpl.subject, html: mailTpl.html }).catch(err => console.error('[Mailer] Password reset email failed:', err));
};
