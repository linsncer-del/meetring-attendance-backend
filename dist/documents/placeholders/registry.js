...
export function getPlaceholdersByCategory(category) {
    return PLACEHOLDER_REGISTRY.filter(p => p.category === category);
}
export function isValidPlaceholder(name) {
    if (name === 'participants' || name === '/participants')
        return true;
    // Allow % for images
    const cleanName = name.replace(/^[#/%]+/, '');
    return PLACEHOLDER_REGISTRY.some(p => p.name === cleanName);
}
export function getAllPlaceholderNames() {
