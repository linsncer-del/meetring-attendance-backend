...
export const verifyDbConnection = async (): Promise<void> => {
  try {
    ...
    if (error) {
      ...
      throw new Error(
        `${error.message} (status: ${status ?? 'unknown'}, code: ${error.code ?? 'none'})`
      )
    }
  } catch (err: any) {
    throw new Error(
      `Could not establish connection to Supabase database. Please check SUPABASE_URL and key settings, and confirm the schema has been applied. Error: ${err.message}`
    )
  }
}