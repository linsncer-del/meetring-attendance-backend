...
// HTML helpers
// ─────────────────────────────────────────────────────────────────────────────
const row = (cells, isEven) => `<tr style="${isEven ? 'background-color:#f9f9f9;' : ''}">${cells.map((c, i) => `<td style="${i === 0 ? 'text-align:center;width:40px;' : ''}">${c}</td>`).join('')}</tr>`;
const headerRow = (headers) => `<tr>${headers.map((h, i) => `<th style="${i === 0 ? 'width:40px;' : ''}">${h}</th>`).join('')}</tr>`;
const formatDate = (d) => {
    if (!d)
        return 'N/A';
    try {
...
export const buildReportHtml = (data) => {
    ...
    const logoUrl = data.organization_profile?.logo_url ?? '';
