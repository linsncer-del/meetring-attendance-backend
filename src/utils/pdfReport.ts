...
const row = (cells: string[], bg = '#fff') =>
  `<tr style="background:${bg}">${cells.map(c => `<td style="padding:8px 12px;border:1px solid #dde3f0;font-size:13px;">${c}</td>`).join('')}</tr>`

const headerRow = (headers: string[]) =>
  `<tr>${headers.map(h => `<th style="padding:10px 12px;background:#003087;color:#fff;text-align:left;font-size:13px;border:1px solid #002070;">${h}</th>`).join('')}</tr>`

const formatDate = (d: string) => new Date(d).toLocaleDateString('en-KE', { year:'numeric', month:'long', day:'numeric' })
const formatTime = (t: string | null | undefined) => {
  if (!t) return 'N/A'
  try { return new Date(t).toLocaleTimeString('en-KE', { hour:'2-digit', minute:'2-digit' }) } catch { return t }
}
