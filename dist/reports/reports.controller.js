...
export const generate = async (c) => {
    try {
        ...
        writeAuditLog(user.id, 'report_generated', `Report generated for meeting: ${meetingId}`, c.req.header('x-forwarded-for') ?? undefined);
...
export const list = async (c) => {
    try {
        ...
        const { reports, total } = await ReportsService.listReports(user.id, user.role, page, limit, c.req.query('status'));
...
export const submitToHR = async (c) => {
    try {
        const user = c.get('user');
        const reportId = c.req.param('id') || '';
        await ReportsService.submitToHR(reportId, user.id);
        writeAuditLog(user.id, 'report_submitted_to_hr', `Report submitted to HR: ${reportId}`, c.req.header('x-forwarded-for') ?? undefined);
